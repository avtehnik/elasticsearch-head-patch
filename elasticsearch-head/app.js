...
(function( app ) {
	...
	data.DataSourceInterface = ux.Observable.extend({
		...
		_getSummary: function(res) {
			this.summary = i18n.text("TableResults.Summary", res._shards.successful, res._shards.total, res.hits.total.value, (res.took / 1000).toFixed(3));
		},
		_getMeta: function(res) {
			this.meta = { total: res.hits.total.value, shards: res._shards, tool: res.took };
		}
	});
...
(function( $, app ) {
	...
	services.Cluster = ux.Class.extend({
		defaults: {
			base_uri: window.localStorage['activeServer']
		},
		init: function() {
...
(function( $, app, i18n ) {
	...
	ui.ClusterConnect = ui.AbstractWidget.extend({
		...
		init: function() {
			...
			this.cluster = this.config.cluster;
			this.el = $.joey(this._main_template());
      this.el.find(".uiClusterConnect-uri").val(window.localStorage['activeServer']);
		...
		_reconnect_handler: function() {
		    var base_uri = this.el.find(".uiClusterConnect-uri").val();
			  window.localStorage['activeServer'] = base_uri;
		...
		_main_template: function() {
			...
			return { tag: "SPAN", cls: "uiClusterConnect", children: [
					{ tag: "SELECT", value: window.localStorage['activeServer'], cls: "uiClusterConnect-uri", onchange:this._reconnect_handler, id: this.id("baseUri"), children: [
							"http://vagrant.cobrowser.io:9200/",
							"http://localhost:9200/"
						].map(ut.option_template) }
			]};
		}
	});
